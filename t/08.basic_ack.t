...
# Test for ackowledging a message
{
    local $@;
    my $res = eval { $ch->basic_get( $queue, { no_ack => 0 } ) };
    ...
    my $ack = $ch->basic_ack( $res->{delivery_tag}, 0 );
    is( $ack, 0, "Acknowledged a message" );
}

# Test for ackowledging all messages at once
{
    local $@;
    my $max = int( rand 10 ) + 1;
    foreach ( 1 .. $max ) {
        $ch->basic_publish(
            {
                exchange    => $exchange,
    ...
    # Try to get all messages published above
    my $data;
    foreach ( 1 .. $max ) {
        $data = $ch->basic_get( $queue, { no_ack => 0 } );
        is( $data->{message_count},
            $max - $_,
            "message_count in response is " . $data->{message_count} );
    }
    # Try to acknowledge all messages at once
    my $res = $ch->basic_ack( $data->{delivery_tag}, 1 );
    is( $res, 0, "Acknowledged $max messages" );

    # Expect undef
    $data = $ch->basic_get( $queue, { no_ack => 0 } );
    is( $data, undef,
        "No message returned, all messages already have been acknowledged" );
}
